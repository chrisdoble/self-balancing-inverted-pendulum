...
So how do we linearise our equations of motion? First, let's rewrite them in vector form. We'll combine our two real-valued functions into one vector-valued function \[\begin{bmatrix}
  ...
  \end{bmatrix}.\] Let's also introduce the state vector $\vec{x}$ which can be used to describe the state of the system and contains the four variables $\theta$, $\dot{\theta}$, $x$, and $\dot{x}$ \[\vec{x} = \begin{bmatrix}
    \theta       \\
    \dot{\theta} \\
    x            \\
    \dot{x}
...
The term $D \vec{f}|_{\vec{p}}$ is the Jacobian matrix of our function evaluated at the point $\vec{p}$. It plays the same role the derivative did in the previous two-dimensional example — namely when multiplied by a vector $\vec{x}$ it tells you how much each component of the function differs from the point at which it was linearised. If we actually evaluate this at the point $\vec{p}$ we get \[D \vec{f}|_{\vec{p}} = \begin{bmatrix}
  ...
  \end{bmatrix}\] which means the linearised version of our function is \[\dvec{x} = \begin{bmatrix}
    0                           & 1 & 0 & 0 \\
    \frac{g (m_1 + m_2)}{l m_1} & 0 & 0 & 0 \\
    0                           & 0 & 0 & 1 \\
    \frac{g m_2}{m_1}           & 0 & 0 & 0
